...
qiniu.conf.ACCESS_KEY = '1IEzgpQTZFqDHhUGPPyESF_dmVrO6-abJGgxlxCJ';
qiniu.conf.SECRET_KEY = 'JMLDUy84Yml9To82mF8HI0onJr0wl_lOzH_jnMjZ';
    ...
    fs.readFile(req.files.img.path, function (err, data) {//使用fs模块readFile打开图片文件
        ...
        fs.writeFile(des_file, data, function (err) {//用fs.writeFile将图片内容写入指定的路径中
            ...
            if (err) {
                ...
                //上传到七牛后保存的文件名
                var key = req.files.img.originalFilename;
...
exports.huoqu = function (req, res) {
